...
(function($) {
    ...
    var Defaults = {
        ...
        titleDays: ['أحد', 'إثنين', 'ثلاثاء', 'أربعاء', 'خميس', 'جمعة', 'سبت'],
        titleMonths: ['يناير', 'فبراير', 'مارس', 'ابريل', 'مايو', 'يونية', 'يوليو', 'اغسطس', 'سبتمبر', 'اكتوبر', 'نوفمبر', "ديسمبر"],
    ...
    $.fn.tDatePicker = function(pr_el, options) {
        ...
        if (options !== undefined) {
			...
			var num_Limit = update_options.limitDateRanges || 327;
        ...
        if (typeof(pr_el) === 'string') {
			...
			var get_startDate = settings.startDate;
        ...
        function setThemeCheckDate(pr_title, pr_class, pr_data_utc, pr_input, pr_fm_input) {
            return '<div class="t-dates t-date-' + pr_class + '"><label class="t-date-info-title">' + pr_title + '</label>' +
		...
		function showThemeDate_cal(pr_class, pr_data_utc) {
			if (pr_data_utc !== 0 && pr_data_utc !== null) {
                var d = new Date(pr_data_utc)
                var showMonths = settings.titleMonths[d.getMonth()].slice(0, settings.titleMonthsLimitShow);
                return '<span class="t-day-' + pr_class + '"> ' + check_num_10(d.getDate()) + '</span>' + '<span class="t-month-' + pr_class + '"> ' + showMonths + ' </span>' + '<span class="t-year-' + pr_class + '"> ' + check_num_10(d.getFullYear()) + '</span>'
                ...
                toDayElement[i].onclick = function(e) {
                    ...
                    if ($(this).parents('.t-check-in').hasClass('t-check-in') === true) {
                        ...
                        if (convertDateUTC(convertFormatDf(setChangeDate)) != get_utc) {
							...
							//$(pr_el).trigger('onChangeCI', alert(new Date(get_utc)));
							
							var n = new Date(get_utc).toLocaleDateString();
							$(pr_el).trigger('onChangeCI', showchkin(new Date(get_utc)));
                        }
                    ...
                    if ($(this).parents('.t-check-out').hasClass('t-check-out') === true) {
                        ...
                        if (convertDateUTC(convertFormatDf(setChangeDate)) != get_utc) {
							...
							//var n2 = new Date(get_utc).toLocaleDateString();
							
							$(pr_el).trigger('onChangeCO', showchkout(new Date(get_utc)));
                        }
						...
						// datepicker = $(this).parents('.t-datepicker').find('.t-check-in')
                        // if (datepicker.length === 0) {
                            // datepicker = $(this_el).find('.t-picker-only')
                            // setTimeout(function() {
                                // if ($('.t-datepicker-day').length !== 0 && settings.autoClose === true) {
		...
		function showchkin(ndate){
			 const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
			 var month=monthNames[ndate.getMonth()];
			 var days=ndate.getDate();
			 var year=ndate.getFullYear();
			 //$('.t-datepicker-open').append('<input type="hidden" id="hid-indt" value="'+days+'-'+month+'-'+year+'" />');
			 //setTimeout(function(){ $("#in-date").text(days+"-"+month+"-"+year);}, 100)
			 var ncd=ndate.toString();
			 //return ncd.substring(0, 10);
			 setTimeout(function(){ $("#in-date").text(ncd.substring(0, 10));}, 100)
		}
		function showchkout(ndate){
			 const monthNames = ["Jan", "Feb", "Mar", "Apr", "May", "Jun", "Jul", "Aug", "Sep", "Oct", "Nov", "Dec"];
			 var month=monthNames[ndate.getMonth()];
			 var days=ndate.getDate();
			 var year=ndate.getFullYear();
			 //$('.t-datepicker-open').append('<input type="hidden" id="hid-outdt" value="'+days+'-'+month+'-'+year+'" />');
			 //setTimeout(function(){ $("#out-date").text(days+"-"+month+"-"+year);}, 100)
			 var ncd=ndate.toString();
			 setTimeout(function(){ $("#out-date").text(ncd.substring(0, 10));}, 100)
		}
		
		function chkindate(){
			if($('.t-input-check-in').val()!='null'){
				var ncd=new Date($('.t-input-check-in').val()).toString();;
				return ncd.substring(0, 10);
			}
			else if(settings.dateCheckIn != null){
				var ncd=new Date(settings.dateCheckIn).toString();;
				return ncd.substring(0, 10);
			}
			else{
				return "...";
		...
		function chkoutdate(){
			if($('.t-input-check-out').val()!='null'){
				var ncd=new Date($('.t-input-check-out').val()).toString();;
				return ncd.substring(0, 10);
			}
			else if(settings.dateCheckOut != null){
				var ncd=new Date(settings.dateCheckOut).toString();;
				return ncd.substring(0, 10);
			}
			else{
				return "...";
        ...
        function getTableCalendar(pr_el, pr_date_utc) {
            if ($(pr_el).find('.t-datepicker-day').length !== 0) {
                ...
                $(pr_el).find('.t-dates').parent().append('<div style="direction:rtl" class="t-datepicker-day' + checkNumCalendar + '"><div class="cal-footer"><div style="position:relative"><span class="in-date" id="in-date">'+chkindate()+'</span><span class="out-date" id="out-date">'+chkoutdate()+'</span></div><button class="btn-close-cal">حدد هذه التواريخ</button></div><div class="header-cal"><div class="con-flx"><span class="dp-close" style="margin-left: 12px;">&#10005;</span><span>حدد تاريخ</span></div></div>' + convertArrayToString(dataTheme) + '</div>')
				}
				else{
					$(pr_el).find('.t-dates').parent().append('<div class="t-datepicker-day' + checkNumCalendar + '"><div class="cal-footer"><div style="position:relative"><span class="in-date" id="in-date">'+chkindate()+'</span><span class="out-date" id="out-date">'+chkoutdate()+'</span></div><button class="btn-close-cal">Select these dates</button></div><div class="header-cal"><div class="con-flx"><span class="dp-close">&#10005;</span><span>Select Date</span></div></div>' + convertArrayToString(dataTheme) + '</div>')
				}
